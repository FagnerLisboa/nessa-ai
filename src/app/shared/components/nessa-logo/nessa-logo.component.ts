...
/**
 * Símbolo da NESSA — borboleta minimalista, geométrica e simétrica.
 * Construída em SVG, sem imagens externas.
 */
@Component({
  selector: "app-nessa-logo",
  standalone: true,
  template: `
    <svg
      class="logo"
    ...
    >
      ...
      <g class="logo__wings" [attr.fill]="'url(#' + gradientId + ')'">
  ...
  styles: `
    ...
    /* Entrada suave — acontece uma única vez, sem loop. */
    ...
    .logo {
      animation: logo-arrive 0.6s cubic-bezier(0.22, 0.61, 0.36, 1) both;
    }
...
export class NessaLogoComponent {
  /** Tamanho (px) do símbolo. */
  readonly size = input<number>(24);

  /** id único para o gradiente, evitando colisão quando há várias instâncias. */
  protected readonly gradientId = `nessa-wing-${logoSeq++}`;
}
